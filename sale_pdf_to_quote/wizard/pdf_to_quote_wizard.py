...
# Series die we herkennen in schema/offerte-PDFs
SERIE_KEYS = ("vwl", "split", "pure", "tower")
...
class PdfToQuoteWizard(models.TransientModel):
    ...
    def _auto_detect_brand(self, text):
        """Heel lichte detectie op basis van bekende productprefixen in de PDF.
        (Wordt enkel gebruikt als restrict_to_brand=True maar brand_filter leeg is.)
        """
        tl = text.lower()
    ...
    def _detect_series_from_text(self, text: str):
        """Zoek over de HELE PDF-tekst naar hoofdproducten (arotherm + serie).

        - We kijken eerst naar regels met 'arotherm' en nemen daar series uit.
        - Als dat niets oplevert: neem series die ergens in de PDF voorkomen.

        Return: set(series_tokens)
        """
        series = set()
        ...
        if series:
            return series
        return self._extract_series_tokens(text or '')
    ...
    def _match_product(self, code=None, desc=None, brand_name=None, schema_mode=False):
        ...
        if desc and len(desc) > 3:
            ...
            # In schema-mode NIET te streng: familie OF maat volstaat (schema's zijn vaak te kort)
            if schema_mode:
                ...
                # zorg dat familie/maat vooraan staan
                if 'vwl' in tokens:
    ...
    def action_create_quotation(self):
        ...
        # Merk bepalen (optioneel)
        brand_name = None
        if self.restrict_to_brand:
            # Let op: brand_filter moet zichtbaar gezet worden in de wizard view (XML).
        ...
        # --------- Schema (bottom_bar): hoofdproducten + bijproducten via serie over HELE tekst ---------
        done_by_series = False
        if pdf_type == 'bottom_bar':
            series = self._detect_series_from_text(text)
            if series:
                _logger.info('Schema series detected in PDF text: %s', ', '.join(sorted(series)))
                Product = self.env['product.product']
                groups = [self._token_group(s) for s in series]
                domain = self._combine_or_groups(groups)
                if brand_name:
                    domain = self._brand_domain(brand_name, for_model='product.product') + domain
                found = Product.with_context(active_test=False).search(domain or [], limit=5000)
                if found:
                    for p in found:
                ...
                unmatched_items.append({'raw': 'No series tokens found in PDF text', 'score': 0, 'candidate': '—', 'method': 'no_series'})
