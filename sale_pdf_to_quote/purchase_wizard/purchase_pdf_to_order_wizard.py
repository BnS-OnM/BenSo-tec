...
class PurchasePdfToOrderWizard(models.TransientModel):
    ...
    _description = "PDF to Purchase Order Wizard"
    ...
    # Result fields
    purchase_order_id = fields.Many2one("purchase.order", string="Created Purchase Order", readonly=True)
    ...
    def action_create_purchase_order(self):
        ...
        _logger.info(
            "Created purchase order %s: %s matched, %s unmatched (brand=%s)",
            purchase_order.name, len(matched_lines), len(unmatched_items), brand_name
        )
